...
public class FXMLDocumentController implements Initializable {
   ...
   @FXML // fx:id="DailyCheckLine"
        ...
        addnightexcel.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                try {                                      
                    ...
                    //findAndDrawAnomalies(allNights);
    ...
    private void findAndDrawAnomalies(ArrayList<EventNight> nights){
        ArrayList<Double> nightsEndex=new ArrayList<>();        
        for (int i=0 ; i<nights.size() ; i++)
            nightsEndex.add(nights.get(i).getEndex());     
        Collections.sort(nightsEndex);
        System.out.println(nightsEndex);                
    ...
    private Pair<Double, Double> findBounderys(ArrayList<Double> nightsEndex) {
        int medianLOc=(int)(nightsEndex.size()/2);
        int q1=medianLOc/2;
        int q3=medianLOc+q1;
        return new Pair<Double,Double>(nightsEndex.get(q1),nightsEndex.get(q3));       
    }
